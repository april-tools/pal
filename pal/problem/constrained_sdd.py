...
class SDDProblem(ConstrainedProblem):
    def __init__(
            self,
            img_id: int,
            sdd_data_path: str,
        ...
        self.dataset = csdd.ConstrainedStanfordDroneDataset(
            img_id=img_id,
            sdd_data_path=sdd_data_path,
            download=download,
        )
        self.window_size = window_size
        self.sampling_rate = sampling_rate
        self.predict_horzon_samples = predict_horizon_samples


    def load_dataset(self, path):
        return self.dataset.get_trajectory_prediction_dataset(
            window_size=self.window_size,
            sampling_rate=self.sampling_rate,
            predict_horizon_samples=self.predict_horzon_sample,
        )
